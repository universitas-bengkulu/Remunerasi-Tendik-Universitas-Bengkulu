...
class JabatanController extends Controller
{
    ...
    public function index(){
        ...
        return view('kepegawaian/jabatan.index',compact('jabatans'));
    }
    ...
    public function post(Request $request){
        ...
        for ($i=0; $i <count($sudah) ; $i++) { 
            if ($request->nm_jabatan == $sudah[$i]->nm_jabatan) {
                return redirect()->route('kepegawaian.jabatan')->with(['error'   =>  'Jabatan Sudah Ditambahkan']);
            }
        }
        Jabatan::create([
        ...
        return redirect()->route('kepegawaian.jabatan')->with($notification);
    }
