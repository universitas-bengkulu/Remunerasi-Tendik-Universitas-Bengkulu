            ...
            <div class="row" style="margin-right:-15px; margin-left:-15px;">
                ...
                <div class="col-md-12" >
                    <nav aria-label="...">
                        <ul class="pagination">
                            <li class="page-item">
                                <a class="page-link" href="{{ route('administrator.rekapitulasi.persentase_absen',[$periode_id]) }}">Previous</a>
                            </li>
                            ...
                            <li class="page-item"><a class="page-link" href="{{ route('administrator.rekapitulasi.persentase_absen',[$periode_id]) }}">6</a></li>
                ...
                {{-- <div class="col-md-12">
                    @if ($a == "sudah")
                        ...
                        @else
                        <a href="{{ route('administrator.rekapitulasi.generate_total_akhir',[$periode_aktif->id]) }}" id="generate" onclick="generateTendik()" class="btn btn-primary btn-sm"><i class="fa fa-cog fa-spin"></i>&nbsp; Generate Total Remunerasi</a>
                    @endif
                        ...
                        <tbody>
                            ...
                            @foreach ($datas as $data)
                                <tr>
                                    ...
                                    <td> Rp. {{ number_format($data->nominal_sanksi_disiplin,2) }} </td>
                                    <td> {{ $data->potongan_integritas_satu_bulan }} </td>
                                    <td> {{ $data->nilai_skp }} </td>
                                    <td> {{ $data->potongan_skp }} </td>
                                    @if ($periode_aktif->jumlah_bulan == 1)
                                        <td>{{ $data->persen_absen_bulan_satu }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_satu,2) }}</td>
                                        @elseif ($periode_aktif->jumlah_bulan == 2)
                                        <td>{{ $data->persen_absen_bulan_satu }}</td>
                                        <td>{{ $data->persen_absen_bulan_dua }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_satu,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_dua,2) }}</td>
                                        @elseif ($periode_aktif->jumlah_bulan == 3)
                                        <td>{{ $data->persen_absen_bulan_satu }}</td>
                                        <td>{{ $data->persen_absen_bulan_dua }}</td>
                                        ...
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_satu,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_dua,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_tiga,2) }}</td>
                                        @elseif ($periode_aktif->jumlah_bulan == 4)
                                        <td>{{ $data->persen_absen_bulan_satu }}</td>
                                        <td>{{ $data->persen_absen_bulan_dua }}</td>
                                        <td>{{ $data->persen_absen_bulan_tiga }}</td>
                                        ...
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_dua,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_tiga,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_empat,2) }}</td>
                                        @elseif ($periode_aktif->jumlah_bulan == 5)
                                        <td>{{ $data->persen_absen_bulan_satu }}</td>
                                        <td>{{ $data->persen_absen_bulan_dua }}</td>
                                        <td>{{ $data->persen_absen_bulan_tiga }}</td>
                                        ...
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_tiga,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_empat,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_lima,2) }}</td>
                                        @elseif ($periode_aktif->jumlah_bulan == 6)
                                        <td>{{ $data->persen_absen_bulan_satu }}</td>
                                        <td>{{ $data->persen_absen_bulan_dua }}</td>
                                        <td>{{ $data->persen_absen_bulan_tiga }}</td>
                                        ...
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_empat,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_lima,2) }}</td>
                                        <td>Rp. {{ number_format($data->nominal_absen_bulan_enam,2) }}</td>
                                    @endif
                                    <td>Rp. {{ number_format($data->total_absensi,2) }}</td>
                                    <td>Rp. {{ number_format($data->total_skp,2) }}</td>
                                    <td>Rp. {{ number_format($data->total_integritas,2) }}</td>
                                    <td>
                                        @if ($data->total_akhir_remun == null)
                                            <a style="color: red">Total belum digenerate</a>
                                            @else
                                            Rp. {{ number_format($data->total_akhir_remun,2) }}
                                        @endif
                                    </td>
