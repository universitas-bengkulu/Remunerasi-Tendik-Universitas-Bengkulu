    ...
    <section class="panel" style="margin-bottom:20px;">
        ...
        <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
            <div class="row" style="margin-right:-15px; margin-left:-15px;">
                ...
                <div class="col-md-12">
                    {{-- @if ($a == "sudah") --}}
                        {{-- <button class="btn btn-primary btn-sm disabled"><i class="fa fa-cog fa-spin"></i>&nbsp; Generate Nominal Potongan SKP</button> --}}
                        {{-- @else --}}
                        <a href="{{ route('kepegawaian.r_skp.generate_nominal_submit',[$periode_id]) }}" id="generate" onclick="generateTendik()" class="btn btn-primary btn-sm"><i class="fa fa-cog fa-spin"></i>&nbsp; Generate Nominal Potongan SKP</a>
                    {{-- @endif --}}
                    <button class="btn btn-warning btn-sm disabled" id="proses-generate" style="display:none;color:white;cursor:pointer;"><i class="fa fa-cog fa-spin"></i>&nbsp; Generate Nominal Potongan SKP</button>
                </div>
                <div class="col-md-12">
                        ...
                        <tbody>
                            ...
                            @foreach ($datas as $key => $skp)
                                <tr>
                                    ...
                                    <td>
                                        @if (empty($skp->jumlah_bulan))
                                            <label class="badge badge-danger"><i class="fa fa-minus"></i></label>
                                            @else
                                            {{ $skp->jumlah_bulan }} Bulan
                                        @endif
                                    </td>
                                    <td>
                                        @if (empty($skp->potongan_skp))
                                            <label class="badge badge-danger"><i class="fa fa-minus"></i></label>
                                            @else
                                            {{ $skp->potongan_skp }}%
                                        @endif
                                    ...
                                    <td>
                                        Rp.{{ number_format($skp->nominal_potongan,2) }}
                                    </td>
